...
class UniqueDict(Generic[K, V]):
    ...
    def __init__(self,
            keychars : Sequence[K] = string.ascii_uppercase,
            keylen : int = 6):
...
class Annotation(Action):
    ...
    def pack(self):
        return {'id': Action.ID_ANNOTATION,
            'text': self.txt,
            'passage_id': self.psg,
            'pos': self.pos,
        }
...
class Layer:
    ...
    def action_add(self, data):
        if ('user_id' in data
        and data['user_id'] in self.updates):
...
class Space:
    ...
    async def user_connect(self, userid : int, usr : User):
        ...
        for un in self.users:
            await self.users[un].update(username, {'msg': MSG_USER_CONNECT, 'user_name': usr.username})
        for psg_id, psg_txt in self.passages.items():
            await usr.update({'msg': MSG_PASSAGE_ADD,
            'passage_id': psg_id,
    ...
    async def user_disconnect(self, username : str):
        for un in self.users:
            await self.users[un].update(username, {'msg': MSG_USER_DISCONNECT, 'user_name': username})
