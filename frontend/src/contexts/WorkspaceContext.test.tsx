...
describe("WorkspaceContext", () => {
  ...
  it("useWorkspace returns updated value when provider re-renders", () => {
    ...
    const { result, rerender } = renderHook(() => useWorkspace(), {
      wrapper: ({ children }) => (
        <WorkspaceProvider value={value1}>{children}</WorkspaceProvider>
      ),
    })
    ...
    rerender({
      wrapper: ({ children }: { children: React.ReactNode }) => (
        <WorkspaceProvider value={value2 as unknown as WorkspaceContextValue}>{children}</WorkspaceProvider>
      ),
    })
