...
export default class Space extends Vue {
  ...
  submitSearch(): void {
    ...
    if (searchText !== null) {
      ...
      if (this.sectionATitle === null) {
        [this.sectionATitle, this.sectionAText] = this.parseSearch(searchText);
      } else if (this.sectionATitle !== this.searchText) {
        [this.sectionBTitle, this.sectionBText] = this.parseSearch(searchText);
      }
      this.searchText = null;
    }
  }
  parseSingleVerse(searchText: string): [string, string] {
    // 1 John 1:1, Genesis 1:1
    return [searchText, textArray[verseIndexer[searchText]]];
  }
  parseSingleChapter(searchText: string): [string, string[]] {
    // 1 John 1, Genesis 1
  ...
  parseAcrossChapters(
    ...
    const firstIndex = verseIndexer[match[1] + " " + match[2] + ":" + match[3]];
    const lastIndex = verseIndexer[match[1] + " " + match[4] + ":" + match[5]];
    return [searchText, textArray.slice(firstIndex, lastIndex)];
  }
  parseMultipleChapters(
    searchText: string,
    match: RegExpMatchArray
  ): [string, string[]] {
    // 1 John 1-2, {Genesis} {1}-{2}
    const firstIndex = verseIndexer[match[1] + " " + match[2] + ":1"];
    const lastIndex = verseIndexer[lastVerse[match[1] + " " + match[3] + ":1"]];
    return [searchText, textArray.slice(firstIndex, lastIndex)];
  }
  parseSearch(
    searchText: string
