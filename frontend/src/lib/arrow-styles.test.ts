...
describe("when inspecting ARROW_STYLES", () => {
  it("then contains four styles", () => {
    expect(ARROW_STYLES).toHaveLength(4);
  });

  it("then has value and label for each style", () => {
  ...
  it("then includes solid, dashed, dotted, double", () => {
...
describe("when using getArrowStyleAttrs", () => {
  it("then returns solid attrs by default", () => {
  ...
  it("then returns solid attrs for 'solid'", () => {
...
describe("when using computeDoubleLinePaths", () => {
  ...
  it("then offsets vertical path horizontally", () => {
  ...
  it("then produces symmetric paths around the center line", () => {
  ...
  it("then handles zero-length segments gracefully", () => {
    const [pathA, pathB] = computeDoubleLinePaths(50, 50, 50, 50, 50, 50, 1.5);
    expect(typeof pathA).toBe("string");
    expect(typeof pathB).toBe("string");
  });

  it("then uses default offset of 1.5", () => {
