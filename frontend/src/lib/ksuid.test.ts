...
describe("when using randomKSUID", () => {
  it("then generates a 27-character string", () => {
    const id = randomKSUID();
    expect(id).toHaveLength(27);
  });

  it("then generates a base62 string", () => {
    const id = randomKSUID();
    expect(id).toMatch(/^[0-9A-Za-z]{27}$/);
  });

  it("then generates unique IDs on each call", () => {
    const ids = new Set(Array.from({ length: 100 }, () => randomKSUID()));
    expect(ids.size).toBe(100);
  });

  it("then sorts chronologically (lexicographic ordering)", () => {
