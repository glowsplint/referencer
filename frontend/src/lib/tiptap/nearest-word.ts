...
function findWordsOnNearestLine(candidates: WordCenter[], direction: "up" | "down"): WordCenter[] {
...
export function findWordInReadingOrder(
  ...
  const currentIndex = sorted.findIndex(
    (w) => w.editorIndex === current.editorIndex && w.from === current.from && w.to === current.to,
  );
...
export function findFirstWordOnAdjacentLine(
  ...
  const beyondLine = candidates.filter((c) =>
    isForward ? c.cy > currentCenter.cy + LINE_TOLERANCE : c.cy < currentCenter.cy - LINE_TOLERANCE,
  );
...
export function findFirstWordOnLine(
  ...
  const sameLine = candidates.filter((c) => Math.abs(c.cy - center.cy) <= LINE_TOLERANCE);
  if (sameLine.length === 0) return null;
...
export function isAtLineStart(
  ...
  return (
    sameEditorCandidates.filter(
      (c) =>
        Math.abs(c.cy - center.cy) <= LINE_TOLERANCE && c.cx < center.cx - SAME_POSITION_THRESHOLD,
    ).length === 0
  );
}
