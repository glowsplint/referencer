...
export function useSelectionScroll(
  ...
  useLayoutEffect(() => {
    if (!editor || editor.isDestroyed || !selection || selection.editorIndex !== editorIndex) {
      return;
    }
    ...
    try {
      ...
      scrollToKeepInView(wrapper, topInContent, leftInContent, wordRect.height, wordRect.width);
    } catch {
