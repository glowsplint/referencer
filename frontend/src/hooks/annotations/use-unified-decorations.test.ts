...
/** Shape returned by the mocked Decoration.inline */
...
describe("useUnifiedDecorations", () => {
  ...
  it("when not locked, then dispatches empty decorations", () => {
    ...
    renderHook(() => useUnifiedDecorations(asEditor(editor), [], 0, false, false));
  ...
  it("when highlights exist in matching editor, then creates decorations for them", () => {
    ...
    renderHook(() => useUnifiedDecorations(asEditor(editor), [layer], 0, true, false));
  ...
  it("when arrows have endpoints in this editor, then creates decorations for them", () => {
    ...
    expect((capturedDecorations[0] as MockDecoration).attrs.class).toBe("arrow-endpoint");
    expect((capturedDecorations[1] as MockDecoration).attrs.class).toBe("arrow-endpoint");
  });
  ...
  it("when highlight and arrow overlap, then segments them into blended pieces", () => {
    ...
    const seg1 = capturedDecorations[0] as MockDecoration;
    const seg2 = capturedDecorations[1] as MockDecoration;
    const seg3 = capturedDecorations[2] as MockDecoration;
  ...
  it("when highlights from different layers overlap, then blends their colors", () => {
    ...
    renderHook(() => useUnifiedDecorations(asEditor(editor), [layer1, layer2], 0, true, false));
    // Breakpoints: 1, 5, 10, 15
