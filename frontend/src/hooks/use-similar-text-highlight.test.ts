...
describe("useSimilarTextHighlight", () => {
  ...
  it("dispatches empty decorations when not locked", () => {
    ...
    renderHook(() =>
      useSimilarTextHighlight(editor as any, selection, 0, false, null, false)
    )

    expect(editor.state.tr.setMeta).toHaveBeenCalledWith(
      similarTextPluginKey,
      DecorationSet.empty
    )
  })
  ...
  it("dispatches empty decorations when no selection", () => {
    ...
    renderHook(() =>
      useSimilarTextHighlight(editor as any, null, 0, true, null, false)
    )
  ...
  it("creates decorations for matches", () => {
    ...
    renderHook(() =>
      useSimilarTextHighlight(editor as any, selection, 0, true, null, false)
    )
  ...
  it("uses 0.15 opacity for highlight color", () => {
    ...
    renderHook(() =>
      useSimilarTextHighlight(editor as any, selection, 0, true, "#fca5a5", false)
    )
