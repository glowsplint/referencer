...
test.describe("arrow drawing", () => {
  test("when two different words are selected and Enter is pressed, then an arrow is drawn", async ({
    page,
  }) => {
  ...
  test("when arrow line is clicked, then it is selected and arrow tool is activated", async ({
    page,
  }) => {
    const firstParagraph = page.locator(".simple-editor p").first();
    const box = await firstParagraph.boundingBox();
  ...
  test("when selected arrow X icon is clicked, then arrow is deleted; hover alone does not show X", async ({
    page,
  }) => {
    const firstParagraph = page.locator(".simple-editor p").first();
    const box = await firstParagraph.boundingBox();
  ...
  test("when anchor is confirmed and arrow keys are used, then target can be navigated to", async ({
    page,
  }) => {
    const firstParagraph = page.locator(".simple-editor p").first();
    const box = await firstParagraph.boundingBox();
  ...
  test("when arrow tool is activated with existing selection, then selection is preserved for navigation", async ({
    ...
    await expect(page.locator(".simple-editor-wrapper.arrow-mode")).toHaveCount(0, {
      timeout: 2000,
    });
