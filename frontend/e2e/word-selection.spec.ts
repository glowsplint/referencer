...
test.describe("word selection in locked mode", () => {
  test("when a word is clicked in locked mode, then word-selection decoration appears", async ({
    page,
  }) => {
  ...
  test("when Escape is pressed after selecting, then word-selection is cleared", async ({
    page,
  }) => {
    // First click a word
    const firstParagraph = page.locator(".simple-editor p").first();
