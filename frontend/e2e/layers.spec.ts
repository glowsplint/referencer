...
test("adding multiple layers increments names", async ({ page }) => {
  ...
  await expect(page.getByTestId("layerName-6")).toHaveText("Layer 3");
});
...
test("clicking a layer makes it active", async ({ page }) => {
  ...
  // Click second added layer (index 5)
...
test("double-click layer name enters edit mode and rename", async ({ page }) => {
  ...
  await expect(page.getByTestId("layerName-4")).toHaveText("Highlights");
});

test("layer swatch opens color picker", async ({ page }) => {
  await page.getByTestId("addLayerButton").click();
  await page.getByTestId("layerSwatch-4").click();
  await expect(page.getByTestId("colorPicker-4")).toBeVisible();
});
...
test("selecting a color closes picker", async ({ page }) => {
  ...
  await page.getByTestId("colorPicker-4").locator("button").first().click();
  await expect(page.getByTestId("colorPicker-4")).not.toBeVisible();
});

test("layer visibility toggle changes title", async ({ page }) => {
  await page.getByTestId("addLayerButton").click();

  const toggle = page.getByTestId("layerVisibility-4");
